...
public class PublicInterfaceImpl implements PublicInterfaceBiz {
    ...
    @Override
    public void publicPostHeaderRequest(Activity context, Map<String, Object> paramsMap, String url, OnRequestListener requestListener) {
        Map<String, String> params=new HashMap<>();
        L.e("Https","Post url = "+url);
        if(null!=paramsMap){
            for (Map.Entry<String, Object> entry : paramsMap.entrySet()) {
//                L.e("Https","Post entry = "+entry.getValue().toString());
                params.put(entry.getKey(),entry.getValue().toString());
            }
        }else{
            paramsMap=new HashMap<>();
        }
//        L.e("Https","header = "+header);
        L.e("Https","Post params = "+new Gson().toJson(params));
        OkHttpUtils.post().url(url)
                .addHeader("Authorization", ""+ MyActivity.userBean().getId())
                .params(params)
                .tag(context)
                .build().execute(new StringCallback() {
            @Override
